...
rule maker_round1:
    ...
    shell:
        ...
        {module}
        {maker}
        export LIBDIR=/work/home/zhangzhiyang/miniconda3/envs/maker/share/RepeatMasker/Libraries
...
rule SNAP_Training:
    ...
    shell:
        ...
        cd ..
        hmm-assembler.pl genome params > ../../../../../../{output.hmm}
        """
...
rule maker_round2:
    input:
        braker_gff = "results/03.annotation/05.braker_{name}/augustus.hints.gff3",
    ...
    shell:
        ...
        cd results/03.annotation/06.maker/round2
        cp ../../../../ctl/maker/round2/*.ctl ./
        sed -i 's#^genome=#genome={params.fasta}#g' maker_opts.ctl
        sed -i 's#^maker_gff=#maker_gff=../../../../{input.gff}#g' maker_opts.ctl
        sed -i "s#^snaphmm=#snaphmm={params.snap}#g" maker_opts.ctl
        sed -i "s#^gmhmm=#gmhm={params.gm}#g" maker_opts.ctl
        sed -i "s#^augustus_species=#agustus_species=braker_{wildcards.name}#g" maker_opts.ctl
        mpiexec -n {threads} maker -base {wildcards.name} maker_bopts.ctl maker_exe.ctl maker_opts.ctl  >&2 2>../../../../{log}
        cd {wildcards.name}.maker.output
        gff3_merge -d ./{wildcards.name}_master_datastore_index.log -o {wildcards.name}.maker.gff
        fasta_merge -d ./{wildcards.name}_master_datastore_index.log -o {wildcards.name}
