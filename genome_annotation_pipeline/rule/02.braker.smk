...
rule hisat_build:
    ...
    shell:
        ...
        {mapping}
        cp ctl/.gm_key ~/
        cp -r /work/home/zhangzhiyang/miniconda3/envs/braker/config/ ./
...
if Isamples:
    ...
    rule braker:
        ...
        shell:
            ...
            {braker2}
            {genemark}
            braker.pl --species=braker_{name} --genome={input.ref} --workingdir={params.DIR} --gff3 --AUGUSTUS_CONFIG_PATH=`pwd`{augustus_conf}  --nocleanup --softmasking --bam={input.bam} --threads {threads}
            """
#        run:
#            bam = ",".join(input.bam)
#            shell("{module} && {genemark} && {braker2} && braker.pl --species=braker_{name} --genome={input.ref} --workingdir={params.DIR} --gff3 --AUGUSTUS_CONFIG_PATH={augustus_conf}  --nocleanup --softmasking --bam={bam} --cores {threads}")
